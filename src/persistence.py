...
Writes analytical tables to PostgreSQL using SQLAlchemy and exports flat Parquet
files for Power BI consumption.
...
def _insert_fato_journeys(connection: Connection, journeys: pd.DataFrame) -> None:
    df = journeys.copy()
    if "channel_path" in df.columns:
        df["channel_path"] = df["channel_path"].apply(
            lambda x: ",".join(x) if isinstance(x, list) else x
        )
    df.to_sql("fato_jornadas", connection, if_exists="replace", index=False)


def _insert_dim_canais(connection: Connection, dimension: pd.DataFrame) -> None:
    dimension.to_sql("dim_canais", connection, if_exists="replace", index=False)


def _insert_resultados(connection: Connection, results_full: pd.DataFrame) -> None:
    results_full.to_sql("resultados_atribuicao", connection, if_exists="replace", index=False)


def _safe_database_target(db_url: str) -> str:
...
def persist(
    ...
    *,
    ...
    safe_url = _safe_database_target(db_url)

    with engine.begin() as connection:
        _insert_fato_journeys(connection, journeys)
        _insert_dim_canais(connection, build_channel_dimension(journeys))
        _insert_resultados(connection, results_full)
...
def persist_all(
    ...
    db_url = os.getenv("DATABASE_URL", "postgresql://user:pass@localhost:5433/marketing_db")
    return persist(
        journeys,
        results_full,
        db_url=db_url,
